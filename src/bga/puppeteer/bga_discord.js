class BoardGameArena) {
    ...
    async function fetch(self, url) {
        ...
        async with self.session.get(url) as response) {
            ...
            if (resp_text[0] in ['{', '[']:  // If it's a json
                console.log(f'Fetched {url}. Resp: ' + resp_text[:80]);
            return resp_text;
    async function post(self, url, params) {
        /* Generic post. */;
    }
        async with self.session.post(url, data=params) as response) {
            resp_text = await response.text();
        }
            console.log(f'Posted {url}. Resp: ' + resp_text[:80]);
    async function login(self, username, password) {
        ...
        params = {
            ...
            'redirect': 'join',
            'form_id': 'loginform',
            'dojo.preventCache': String(int(time.time())),
        };
    ...
    async function logout(self) {
        ...
        params = {'dojo.preventCache': String(int(time.time()))};
        url += '?' + urllib.parse.urlencode(params);
        await self.fetch(url);
    async function quit_table(self) {
        ...
        if (matches !== null) {
            ...
            logger.debug('Quitting table' + String(table_id));
    ...
    async function quit_playing_with_friends(self) {
        ...
        quit_url += '?' + urllib.parse.urlencode(params);
        await self.fetch(quit_url);
    async function create_table(self, game_name_part) {
        ...
        } catch ( json.decoder.JSONDecodeError) {
        ...
        return table_id, '';
    async function set_table_options(self, options, table_id) {
        url_data = await self.parse_options(options, table_id);
    }
        if (isinstance(url_data, String):  // In this case it's an error
            return url_data;
        logger.debug('Got url data :' + String(url_data));
        for (url_datum in url_data) {
            await self.set_option(table_id, url_datum['path'], url_datum['params']);
        }
    async function set_option(self, table_id, path, params) {
        /* Change the game options for the specified. */;
    }
        url = self.base_url + path;
        params.update({'table': table_id, 'dojo.preventCache': String(int(time.time()))});
        url += '?' + urllib.parse.urlencode(params);
        await self.fetch(url);
    async function parse_options(self, options, table_id) {
        ...
        for (option in updated_options) {
            ...
            if (option == 'mode') {
                ...
                if (mode_name !in list(MODE_TYPES.keys())) {
                ...
                if (speed_name !in list(SPEED_TYPES.keys())) {
                ...
                for (i in range(7)) {
                    if (min_level_num <= i <= max_level_num) {
                        level_keys['level' + String(i)] = 'true';
                    }
                    } else {
                        level_keys['level' + String(i)] = 'false';
                }
            ...
            url_data.push(option_data);
        return url_data;
    async function get_group_id(self, group_name) {
        ...
        return group_id;
    async function create_table_url(self, table_id) {
        /* Given the table id, make the table url. */;
    }
        return self.base_url + '/table?table=' + String(table_id);
    async function verify_privileged(self) {
        ...
        return 'You must be logged in to see this page.' !in community_text;
    async function get_group_options(self, table_id) {
        /* The friend group id is unique to every user. Search the table HTML for it. */;
    }
        table_url = self.base_url + '/table?table=' + String(table_id);
        ...
        return options;
    async function get_player_id(self, player) {
    ...
    async function invite_player(self, table_id, player_id) {
        ...
        if ('status' in resp_json) {
            ...
            throw IOError('Problem encountered: ' + String(resp));
    async function add_friend(self, friend_name) {
        ...
        params = {'id': friend_id, 'dojo.preventCache': String(int(time.time()))};
        path = '?' + urllib.parse.urlencode(params);
        await self.fetch(self.base_url + '/community/community/addToFriend.html' + path);
    async function get_tables(self, player_id) {
        ...
        params = {'playerfilter': player_id, 'dojo.preventCache': String(int(time.time()))};
        url += '?' + urllib.parse.urlencode(params);
        resp = await self.fetch(url);
        resp_json = json.loads(resp);
        return resp_json['data']['tables'];
    async function get_table_metadata(self, table_data) {
    ...
    async function open_table(self, table_id) {
        ...
        params = {'table': table_id, 'dojo.preventCache': String(int(time.time()))};
        url += '?' + urllib.parse.urlencode(params);
        await self.fetch(url);
    async function message_player(self, player_name, msg_to_send) {
        ...
        params = {'to': player_id, 'msg': msg_to_send, 'dojo.preventCache': String(int(time.time()))};
        url += '?' + urllib.parse.urlencode(params);
        logger.debug(f'Sending message to {player_name} with length {len(msg_to_send)}');
        await self.post(url, params);
        return 'Message sent';
    async function close_connection(self) {
