/* Unofficial typescript API for BGA
2021-06-10

# Todo
* [X] Demonstrate login
* [X] Take a screenshot of logged in page

So the way that this should work is that this provides a 
...
const fs = require('fs')
import {modesJSON, speedsJSON, karmasJSON, levels} from "./options";
...
async function main() {
  ...
  let yucata_password = process.env.YUCATA_PW as string
...
*/
async function loginBGA(browser: Browser, username: string, password: string): (Promise<[Page, string]>) {
    ...
    if (responseJson['data']['success']) {
        ...
        fs.writeFile(username + '.txt', responseBody, (err: string) => {
...
*/
async function loginYucataGUI(browser: Browser, username: string, password: string): (Promise<[Page, string]>) {
    const page: Page = await browser.newPage();
